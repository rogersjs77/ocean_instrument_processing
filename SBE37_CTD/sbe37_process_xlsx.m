...
%
% this file takes SBE37 .asc files, with 
...
function []=sbe37_process_xlsx(filter_time,start_time,end_time)
g=9.81;
%% load data files

files=dir('SBE37*.xlsx');
files_header = dir('Header*.asc');
if isempty(files)
   disp('no files found, check format SBE37_XXMAB_...asc')
   return
end
...
% 
% filename_header=files_header(ii(i)).name;
% fid = fopen(filename_header);
% sbe37.hdr{i}=textscan(fid,'%s');
% 
% j=1;
% start_year=[];
...
sbe37.temp{i}=DATA(indx,1)';
sbe37.cond{i}=DATA(indx,2)';
sbe37.sal{i}=DATA(indx,4)';
sbe37.press{i}=DATA(indx,3)';
sbe37.press_Pa{i} = sbe37.press{i}*1E4;

date = TEXT(indx,5)';
hour = TEXT(indx,6)';
for ii=1:length(date)
datestring{ii} = [date{ii} hour{ii}];
end

sbe37.mtime{i}=datenum(datestring,'dd mmm yyyy HH:MM:SS'); % convert julian dats to mtime
%%
