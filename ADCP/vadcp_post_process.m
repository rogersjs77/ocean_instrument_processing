...
function [ ] = vadcp_post_process(bottom_trim,depth_max,filter_time,...
    bad_beam,P_atm,Waves,nfft,fcutoff,dirmethod,corr_min,theta,head_height,...
    extrap,z0,d,time_start,time_end,sample_size,time_bad_vel,head_heading)


%% Combine data from multiple files
filesrdr=dir('vADCP*.mat');
ii=1;
filenamerdr=filesrdr(ii).name;  
load(filenamerdr)

% there is only one structure file, need to rename to adcp
a = whos;
...
% rotate data to ENU
% head_heading = 135;
x_heading = head_heading-90;
adcp.theta = 90 - x_heading;

[adcp.east_vel,adcp.north_vel] = adcp_rotation(adcp.x_vel,adcp.y_vel,-adcp.theta);
adcp.vert_vel = adcp.z_vel;

% remove some large data, not needed
% adcp = rmfield(adcp,{'s','x_vel_s','y_vel_s','z_vel_s','z_vel'});
...
% calculate pressure from adcp.depth or surface track;
if   length(adcp.surface_track)~=length(adcp.mtime)
adcp = rmfield(adcp,'surface_track');    
% pull out depth from velocity data
    for i=1:length(adcp.mtime)
       indx = max(find(~isnan(adcp.y_vel(:,i))));
       if ~isempty(indx) % depth at top of last good cell
           adcp.surface_track(i) = 0.5*(adcp.config.ranges(indx+1)+adcp.config.ranges(indx));
       else
           adcp.surface_track(i) = nan;
       end
     end
end

adcp.depth = adcp.surface_track;
adcp.depth(adcp.depth==0) = nan;

adcp.press_Pa = 1022*9.81*adcp.depth;
...
time_start=adcp.mtime(1);
time_end=adcp.mtime(end);
end
...
S=nanmean(adcp.salinity); g=9.81;
[~,sigma] = swstate(S,nanmean(adcp.temperature),nanmean(press_Pa)*1E-4);
rho = 1000+sigma;
depth = interp1(adcp.mtime,adcp.depth,mtime,'nearest',NaN); % hydrostatic depth above bottom
Depth = nanmean(depth);
...
bin_MAB_orig = adcp.config.ranges(:,1)'+head_height;
...
bins_drop_top = length(find(nansum(adcp.x_vel,2)==0))-1;
...
% remove outliers
t1 = reshape(vel_east2,[],1);
t1 = t1(~isnan(t1));
indx = vel_east2 < quantile(t1,0.01) | vel_east2 > quantile(t1,0.99);
vel_east2(indx)=nan;

t1 = reshape(vel_north2,[],1);
t1 = t1(~isnan(t1));
indx = vel_north2 < quantile(t1,0.01) | vel_north2 > quantile(t1,0.99);
vel_north2(indx)=nan;

t1 = reshape(vel_vert2,[],1);
t1 = t1(~isnan(t1));
indx = vel_vert2 < quantile(t1,0.01) | vel_vert2 > quantile(t1,0.99);
vel_vert2(indx)=nan;
...
for i=1:length(mtime_filt)
   indx = bin_MAB > (depth_filt(i)-bins_from_surf*(bin_MAB(2)-bin_MAB(1)));
